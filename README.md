...
The blocktables and the blocks are considered a BTree data structure, a B+Tree where all data is in the leaves. The blocs are the leafs, the 3 blocktables are the 3 roots. Accordingly, BlockTableEntries must be ordered by their first-id values and the local-ids inside the blocks must be ordered as well.
